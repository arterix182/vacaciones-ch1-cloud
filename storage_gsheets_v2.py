...
# storage_gsheets_v2.py — robust Google Sheets persistence (self-healing)
...
def _ws(name: str, expected_headers):
    ...
    try:
        ...
        ws = sh.add_worksheet(title=name, rows=2000, cols=16)
...
def append_agenda_row(rec: dict):
    ws = _ws("agenda", AGENDA_HEADERS)
    ws.append_row([rec.get("numero",""), rec.get("nombre",""), rec.get("equipo",""),
                   rec.get("fecha",""), rec.get("tipo","")], value_input_option="USER_ENTERED")
