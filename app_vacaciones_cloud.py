...
# Vacaciones CH-1 (Cloud) — Streamlit + Google Sheets
# Persistencia 100% en Google Sheets (multiusuario, 24/7 en Streamlit Cloud)
# Reglas: max 3 por día + no dos del mismo equipo el mismo día
# Admin: exportar Excel/CSV + eliminar registros (solo con admin_password en secrets)
    ...
    if df_dia.empty:
        ...
        if st.session_state.is_admin:
            ...
            if st.button("Eliminar seleccionados", type="primary", key="btn_delete"):
                if to_delete:
                    # Reemplazar agenda con el DF original sin los seleccionados de ese día
                    df_all = load_agenda_df().copy()
                    # Construir etiqueta para todo el DF
                    ...
                    # Guardar (sobrescribe hoja Agenda)
